...
export default function TaxManagementPage() {
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["vehicles", "tax-management"],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-[36px] font-serif font-medium text-[#111111] tracking-tight">Tax Management</h1>
          <p className="text-[18px] text-[#777777] mt-1 mb-4">Monitor vehicle tax status, validity, payments and compliance.</p>
      ...
      <div className="w-full">
        ...
        {isError ? (
          <div className="bg-white border border-[#E5E5E5] rounded-[16px] p-12 shadow-sm flex flex-col items-center text-center">
            ...
            <h3 className="text-[18px] font-semibold text-[#111111] mb-2">Unable to Load Tax Information</h3>
            <p className="text-[15px] text-[#777777] max-w-md mb-6">We couldn't retrieve tax information from Motor Vehicle records. Please check your connection and try again.</p>
            <button 
              onClick={() => refetch()}
